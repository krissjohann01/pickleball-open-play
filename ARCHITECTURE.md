...
**Mid-session changes to time, courts, and cost**: real open-play sessions aren't always fixed — a venue might free up an extra court, or let the group stay longer, after play has already started. Three pure functions in `matchmaking.ts` handle this, all admin-only messages: `extendSession(session, additionalMinutes)` just adds to `durationMinutes` (the live countdown in `SessionView` is derived from `startedAt + durationMinutes`, so extending takes effect immediately without touching `startedAt`). `addCourt(session)` appends a new empty `CourtSlot` (next sequential `courtNumber`) — it doesn't auto-fill it; the admin starts its first game the same way as any court, via "Start Game". `removeCourt(session, courtNumber)` deletes a specific court by its `courtNumber` and always keeps at least one court (a no-op if only one remains); if that court had an active game, its `playerIds` simply stop appearing in `session.courts`, so `getWaitingPool` treats those players as free again on the next matchmaking pass — no explicit "return to waiting" step needed, and their `gamesPlayed` count isn't rolled back (they already played that game). None of these functions know or guess at pricing — `updateSessionCost(session, courtRentalTotal, entranceFeePerPerson)` just overwrites both totals directly, since only the admin knows what the venue actually charged for the added time/court; the UI for this lives as an "Edit costs" toggle in `CostSplit.tsx` (admin-only; hidden entirely from non-admins once there's nothing to show).
